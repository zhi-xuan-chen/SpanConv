...
ckpt = "./Weights/wv3/weight-680.pth" # choose a model
...
def test(file_path):
    ...
    with torch.no_grad():
        ...
        for index in range(num_exm):  # save the LightNet results for matlab evaluate code
            file_name = "LightNet_full_wv3_" + str(index) + ".mat"
            directory_name = "./Results/64_full_wv3_256x256"

            save_name = os.path.join(directory_name, file_name )
            sio.savemat(save_name, {'LightNet_full_wv3_' + str(index): sr[index, :, :, :]})
...
if __name__ == '__main__':
    ...
    file_path = "./test_data/64_full_wv3_256x256.h5"
    test(file_path)
